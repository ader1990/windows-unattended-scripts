...
if (Should-Run-Step "Prepare")
{
    ...
    New-ItemProperty -Path 'HKLM:\SOFTWARE\Unattended' -Name dnsip -Value $dnsip
    ...
    {
        log("Install MSSQL SERVER 2012 at next logon")
        Restart-And-Resume $script "Install"
    }
}

if ($getParams){
    log("Getting the user data")
    ...
    $dnsip=(Get-ItemProperty -Path 'HKLM:\SOFTWARE\Unattended' -Name dnsip).dnsip
}
